...
object ReleaseNotesParser {
    ...
    private fun groupBulletsByArea(bullets: List<String>): List<ReleaseNotesGroup> {
        ...
        bullets.forEach { bullet -> grouped[categorizeBullet(bullet)]?.add(bullet) }
    ...
    private fun categorizeBullet(bullet: String): String {
        ...
        return when {
            normalized.contains("record") || normalized.contains("audio") ||
                normalized.contains("microphone") -> "Recording"
            normalized.contains("openai") || normalized.contains("transcrib") ||
                normalized.contains("transform") || normalized.contains("prompt") ||
                normalized.contains("api key") || normalized.contains("ai") -> "AI & Processing"
            normalized.contains("screen") || normalized.contains("icon") ||
                normalized.contains("button") || normalized.contains("history") ||
                normalized.contains("profile") || normalized.contains("settings") ||
                normalized.contains("release note") || normalized.contains("popup") -> "UI & Workflow"
            normalized.contains("build") || normalized.contains("service") ||
                normalized.contains("version") || normalized.contains("windows") ||
                normalized.contains("sdk") || normalized.contains("fix") -> "Platform & Reliability"
            else -> "Other"
        }
    }
