...
* no user-visible change: market research's new cross-screen footer looked like it survived navigating away from the item, but the research itself was still cancelled the moment you did — caught by review before this shipped
...
* removed the Gemma 3n on-device model option added in 1.28.0 — confirmed on a real device to fail with HTTP 401, since its HuggingFace repo gates the actual file behind a license click-through no matter the filename; re-added once a working unauthenticated source exists
...
* shows whether it's refining a listing or analyzing a photo
