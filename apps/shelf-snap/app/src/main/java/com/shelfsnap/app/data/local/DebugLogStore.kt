 ...
 * photo bytes or full prompt/response/page text, only short summaries.
 *
 * A flat union of fields across every [type], not a sealed class — Gson has no ceremony either
 * way, but a flat shape keeps [DebugLogStore] itself type-agnostic and matches Scrybe/PriceDrop's
 * identical entry shape.
 *
...
class DebugLogStore
    ...
    constructor(
        ...
        private val entryListType = object : TypeToken<List<DebugLogEntry>>() {}.type
        private val legacyCrashListType = object : TypeToken<List<LegacyCrashLogEntry>>() {}.type
        private val legacyAiCallListType = object : TypeToken<List<LegacyAiCallDebugEntry>>() {}.type
                ...
                runCatching {
                    ...
                    val migratedCrashes =
                        runCatching {
                            ...
                            val legacy =
                                gson.fromJson<List<LegacyCrashLogEntry>>(legacyCrashFile.readText(), legacyCrashListType)
                                    ?: emptyList()
                            legacy.map {
                                DebugLogEntry(
                    ...
                    val migratedAiCalls =
                        runCatching {
                            ...
                            val legacy =
                                gson.fromJson<List<LegacyAiCallDebugEntry>>(legacyAiCallFile.readText(), legacyAiCallListType)
                                    ?: emptyList()
                            legacy.map {
                                DebugLogEntry(
                                    timestampMs = it.timestampMs,
