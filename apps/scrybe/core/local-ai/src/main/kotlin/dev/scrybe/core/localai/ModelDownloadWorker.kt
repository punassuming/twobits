 ...
 * (the previous approach) can't guarantee that. Uses the same Hilt [EntryPoint] pattern PriceDrop's
 * `PriceCheckWorker` follows, so no custom `WorkerFactory` is needed.
 */
