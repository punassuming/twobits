...
| **Fully local** | Whisper tiny/base/small via Sherpa-ONNX for transcription; Qwen3 via LiteRT-LM for transforms. Zero network calls. Download models once (150 MB – 2.5 GB), then everything runs on-device. |
...
| `:core:transforms` | Core | `TransformationProvider`, `TransformationPipeline`, three built-in `DefaultProfiles`, Qwen3 local provider |
...
| LiteRT-LM | — | On-device Qwen3 transforms |
...
| **Fully local** | No key or subscription needed. Tap **Download models** in Settings → API Configuration to download Whisper (tiny/base/small, 150 MB – 1.5 GB) and Qwen3 (~2.5 GB). Everything runs on-device. |
