...
class OpenAiProfileSuggestionService @Inject constructor(
    ...
    suspend fun suggestProfile(
    ...
    ): Result<ProfileSuggestion> = runCatching {
        withContext(Dispatchers.IO) {
            ...
            val outputText = parsed.outputText?.takeIf { it.isNotBlank() }
                ?: parsed.output
                    .orEmpty()
            ...
            val suggestion = json.decodeFromString(
                SuggestedProfilePayload.serializer(),
                unwrapJsonEnvelope(outputText),
            )
            val normalizedSteps = suggestion.steps
                .map { it.trim() }
