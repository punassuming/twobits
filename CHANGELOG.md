...
* fix Gemma downloads requiring HuggingFace authentication — `litert-community/Gemma2-2B-IT` is a gated repository; a HuggingFace token field is now shown in Settings > AI Provider (Local) so users can enter their token; the download request sends `Authorization: Bearer` when a token is configured
...
* apply mode-specific accent color to mode badge — session card and recording mode badges now use each mode's own color for icon, text, and background tint instead of the generic secondary container
* merge speakers, tasks, and tags into a single FlowRow — session card now shows a combined footer row where speakers, tasks chip, and tag pills wrap together; tasks chip uses tertiary container colors matching the glow palette
* increase mini waveform height to 28 dp — matches design intent for the session card waveform thumbnail
